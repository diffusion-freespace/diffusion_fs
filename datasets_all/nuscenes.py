...
class NuScenes:
    ...
    def __init__(self,
        ...
        for map_record in self.map:
            ...
            # map_record['mask'] = MapMask(osp.join(self.table_root, map_record['filename']), resolution=map_resolution)
    ...
    def render_future_footprint(self,
                            sample_token: str,
        ...
        return self.explorer.render_future_footprint(sample_token,
                                            camera_channel,
                                            alpha,
                                            patch_radius,
                                            min_polygon_area,
...
class NuScenesExplorer:
    ...
    def render_future_footprint(self,
                            ...
                            min_polygon_area: float = 1000,
                            render_behind_cam: bool = True,
                            render_outside_im: bool = False,
                            layer_names: List[str] = None,
                            verbose: bool = True,
                            out_path: str = None):
        """
        Generate the future footprint limited by the obstacle mask, for the given sample token.
        
        Note that we use the 3D bounding boxes to limit the future footprint as it is directly available here, but it is equivalent 
        to use the 2D segmentation masks of the obstacles.
        ...
        curr_time = self.get_current_timestep(nusc, sample_token)
        ...
        h, w, _ = im_np.shape
        im_size = im.size
        output_mask = np.zeros((h, w)).astype(np.uint8)
        ...
        ego_rotation = Quaternion(poserecord['rotation'])
        ...
        all_other_dims = []
        for box in boxes:
            corners_global = box.bottom_corners()[:2]
            all_other_rotations.append(box.rotation_matrix[:2, :2].T)
            all_other_centers.append(box.center[:2])
            all_other_dims.append(box.wlh[:2])
            all_other_boxes.append(corners_global)
            
        all_other_boxes = np.array(all_other_boxes)
        all_other_rotations = np.array(all_other_rotations)
        all_other_centers = np.array(all_other_centers)
        all_other_dims = np.array(all_other_dims)
        
        map_poses = []
        sample_tokens = nusc.field2token('sample', 'scene_token', sample_record['scene_token'])
        for sample_token in sample_tokens:
            sample_record = nusc.get('sample', sample_token)
            ...
            # Calculate the pose on the map and append.
            ego_position = np.array(pose_record['translation'][:2])
            ego_rotation = Quaternion(pose_record['rotation']).yaw_pitch_roll[0]
            # map_poses.append(pose_record['translation'])
            map_poses.append((ego_position[0], ego_position[1], ego_rotation))

        map_poses = np.array(map_poses[curr_time:])
        if(len(all_other_centers)):
            diff = (map_poses[:, :2][None] - all_other_centers[:, None]).transpose(0, 2, 1)
            rotated_points = (all_other_rotations @ diff).transpose(0, 2, 1)
            m1 = rotated_points[:, :, 0] > -all_other_dims[:, 1][:, None]/2
            m2 = rotated_points[:, :, 0] < all_other_dims[:, 1][:, None]/2
            m3 = rotated_points[:, :, 1] > -all_other_dims[:, 0][:, None]/2
            m4 = rotated_points[:, :, 1] < all_other_dims[:, 1][:, None]/2
            m = (m1 * m2 * m3 * m4).transpose(1, 0).sum(-1) > 0
            try:
                limit = np.nonzero(m)[0][0]
            except:
                limit = len(map_poses)
            print(np.nonzero(m), limit, cam_path)
        else:
            limit = len(map_poses)
        map_poses = map_poses[:limit]
        if(len(map_poses)):
            map_poses = self.interpolate_points(map_poses)
        else:
            return output_mask, im_np, cam_path, map_poses
        boxes = [self._get_bounding_box(pose) for pose in map_poses]
        

        for future_footprint in boxes:
        # for other_box in all_other_boxes:
            points = future_footprint.T
            # points = other_box
            ...
            # inside = np.logical_and(inside, points[1, :] < im.size[0] - 1)
            inside = np.logical_and(inside, points[0, :] > 1)
            # inside = np.logical_and(inside, points[0, :] < im.size[1] - 1)
            if np.any(np.logical_not(inside)):
                continue

            points = points[:2, :]
            cv2.drawContours(output_mask, [points.astype(np.int32).T], contourIdx=-1, color=1, thickness=-1)

        return output_mask, im_np, cam_path, map_poses
    
    def get_current_timestep(self, nusc, sample_token):
        sample_record = nusc.get('sample', sample_token)
        ...
        while current_sample_token != sample_token:
            ...
            next_sample_token = current_sample_record['next']
            position += 1
            current_sample_token = next_sample_token
        
        return position
    
    def interpolate_points(self, points, num_new_points=5):
        interpolated_points = [points[0]]
        
        for i in range(1, len(points)):
            p1 = points[i - 1, :2]
            p2 = points[i, :2]
            new_points = np.linspace(p1, p2, num=num_new_points+2)[1:-1]
            new_points = np.concatenate([new_points, np.ones((new_points.shape[0], 1)) * points[i-1, 2]], -1)
            interpolated_points.extend(new_points)
            interpolated_points.append(points[i])
        
        return np.array(interpolated_points)
    
    def _get_ego_dimensions(self):
        """
        Get the width, height, and length of the ego vehicle.
        """
        ego_length = 4.084  # in meters
        ego_width = 1.730   # in meters
        ego_height = 1.562  # in meters (optional, not used for 2D projection)

        return ego_width, ego_height, ego_length
    
    def _get_bounding_box(self, waypoint):
        """
        Compute the bounding box of the ego vehicle at a given waypoint.
        The bounding box is computed using the vehicle's width, length, and rotation.
        """
        
        width, height, length = self._get_ego_dimensions()
        x, y, heading = waypoint
        # x, y = waypoint
        half_width = width / 2
        half_length = length / 2

        # Bounding box corners relative to the vehicle center
        corners = np.array([
        ...
        # Rotate the corners by the vehicle's heading (yaw)
        ...
        bounding_box = rotated_corners + np.array([x, y])
        # bounding_box = corners + np.array([x, y])
